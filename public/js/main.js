        ...
        let ejsTemplate = `
                <% todos.forEach( (todoObj, index) => { %>
                    ...
                    <div class="group flex items-center ml-2">
                        <input type="checkbox" id="<%= id %>" name="<%= id %>" value="<%= id %>" class="text-gray-500 w-6 h-6 rounded-md self-start sm:mt-1.5">
                        <label for="<%= id %>" class="ml-2 sm:ml-3 checked-sibling:opacity-50 checked-sibling:line-through text-lg leading-snug sm:text-3xl"><%= todoObj.todo %> </label>
